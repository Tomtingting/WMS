...
for($j=2;$j<=$highestRow;$j++)
{
    ...
    //var_dump ($strs);
    //die();
    $sql_p="select * from tbzjzk where ItemId='".$strs[0]."'";
    $result=mysql_query($sql_p);
    $rowcount=mysql_num_rows($result);
    if($rowcount==0){
        $sql_s="insert into tbzjzk(ItemId,Count) values('".$strs[0]."','".$strs[1]."')";
        $result1=mysql_query($sql_s);
        echo $sql_s;
    }
    else{
    $sql ="update tbzjzk set Count='".$strs[1]."' where ItemId='".$strs[0]."'";
    //echo $sql;
